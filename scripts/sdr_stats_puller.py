...
def get_average_icp_fit_score(sdr_id):
    results = db.session.execute('''
        ...
        and prospect.client_sdr_id = ''' + str(sdr_id) + '''
...
def get_message_ops_stats(sdr_id):
    results = db.session.execute('''
        ...
        where prospect.client_sdr_id = ''' + str(sdr_id) + '''
            and prospect_status_records.created_at > NOW() - '7 days'::INTERVAL
        group by 4;
    ''').fetchall()[0]
...
def get_message_ops_targets(sdr_id):
    results = db.session.execute('''
            ...
            from client_sdr 
            where client_sdr.id = ''' + str(sdr_id) + '''
    ''').fetchall()[0]
    return {
...
def get_cta_results(sdr_id):
    results = db.session.execute('''
     select 
        ...
        cast(count(distinct prospect_status_records.prospect_id) filter (where prospect_status_records.to_status = 'ACCEPTED') as float) / count(distinct prospect_status_records.prospect_id) filter (where prospect_status_records.to_status = 'SENT_OUTREACH') "% Acceptance Rate",
        cast(count(distinct prospect_status_records.prospect_id) filter (where prospect_status_records.to_status = 'ACTIVE_CONVO') as float) / count(distinct prospect_status_records.prospect_id) filter (where prospect_status_records.to_status = 'SENT_OUTREACH') "% Conversation Rate"
        from client_sdr   
...
def get_icp_fit_status(sdr_id):
    good_status = get_average_icp_fit_score(SDR_ID) >= 3
    if good_status:
        return '🟢'
    else:
        return '🔴'
...
def get_message_ops_status(sdr_id):
    ...
    if stats['Linkedins Sent in Last 7 Days'] >= targets['Linkedin Goal Volume'] and stats['Emails Sent in Last 7 Days'] >= targets['Email Goal Volume'] and stats['Avg. Time to First Bump'] <= targets['Speed to Lead'] and stats['Auto Bumps Active'] == targets['Auto Bumps Enabled']:
        return '🟢'
    else:
        return '🔴'
...
print(get_icp_fit_status(SDR_ID), "Prospect Fit")
print(get_message_ops_status(SDR_ID), "Message Volume")
print(get_cta_status(SDR_ID), 'Message Quality')
print(sdr_slas_status(SDR_ID), 'SDR SLAs')
"""
)
