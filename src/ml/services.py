...
def handle_chat_icp(chat_content: List[Dict[str, str]], prompt: str, current_csv: str) -> Dict[str, str]:
    """
    ...
    # print('current csv is', current_csv)

    # print('current csv', current_csv)
    # Define the role of the bot for updating the CSV
    system_message_csv = {
        "role": "system",
        "content": '''You are a CSV update assistant. Your task is to update the CSV based on the user's input.
        '''
    }

    # Define the role of the bot for generating the response
    system_message_response = {
        "role": "system",
        "content": '''You are a sales segment creation assistant. Guide the user through creating sales segments 
        by asking relevant questions. Please use newlines in your response, and also, please keep the response at most 3 sentences.
        '''
    }
    ...
    # Prepare the messages for the CSV update
    messages_csv = [system_message_csv] + [{"role": "user", "content": msg.get("sender") +': ' + msg.get("query", "")} for msg in chat_content]
    messages_csv.append({"role": "user", "content": '''User's last message: {prompt}. Here is the current CSV, which may have been edited by the user, in which case you need to honor that:\n{current_csv}. There may be no changes needed for the CSV. You should output a CSV with sales segment(s) detailing the following columns: 1. Segment name 2. segment description 3. value proposition. Only respond with the CSV, nothing else. Go:'''.format(prompt=prompt, current_csv=current_csv)})

    # Call the wrapped_chat_gpt_completion function for CSV update
    csv_content = wrapped_chat_gpt_completion(
        model='gpt-4o',
    ...
    # Remove headers and any empty lines from the CSV content
    csv_lines = csv_content.split('\n')
    csv_lines = [line for line in csv_lines if line.strip() and "segment name" not in line.lower()]
    csv_content = '\n'.join(csv_lines)

    # print('csv content', csv_content)

    # Prepare the messages for the chat response
    messages_response = [system_message_response] + [{"role": "user", "content": msg.get("sender") +': ' + msg.get("query", "")} for msg in chat_content]
    messages_response.append({"role": "user", "content": '''User's last message: {prompt}.'''.format(prompt=prompt)})

    # Call the wrapped_chat_gpt_completion function for chat response
    response_content_response = wrapped_chat_gpt_completion(
        model='gpt-4o',
    ...
    response = response_content_response.strip()

    return {"response": response, "csv": csv_content}
