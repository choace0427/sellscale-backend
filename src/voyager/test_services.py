...
from src.voyager.test_client import LinkedInClient
...
def make_search(keyword, years_of_experience):
    ...
    url = client.get_people_search_url(keyword, company_size, region, years_of_experience, tenure)
    response = sync_page(client, url, 25, company_size, region, years_of_experience, tenure, 0)
...
def sync_page(client: LinkedInClient, url, page_size, company_size, region, years_of_experience, tenure, start):
            ...
            if record.get("currentPositions", None):
                for companies in record.get("currentPositions"):
                    if companies.get("companyUrn", None):
                        print(int(companies["companyUrn"].replace("urn:li:fs_salesCompany:", "")))
