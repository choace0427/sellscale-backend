...
def send_autogenerated_bumps():
    ...
    for sdr in autobumpable_sdrs:
        ...
        if prospect is None or (prospect.li_last_message_from_prospect is not None and sdr.disable_ai_on_prospect_respond):
            continue
        ...
        if sdr.disable_ai_on_message_send:
            convo: List[LinkedinConversationEntry] = LinkedinConversationEntry.li_conversation_thread_by_prospect_id(oldest_auto_message.prospect_id)
            human_sent_msg = next((msg for msg in convo if not msg.ai_generated and msg.connection_degree == 'You'), None)
...
def send_autogenerated_bump(
    ...
    success, violations: tuple[bool, list[str]] = run_autobump_firewall(generated_message_auto_bump_id)
    if not success:
        db.session.delete(message)
        db.session.commit()
        send_slack_message(
