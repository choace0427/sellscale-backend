...
def send_autogenerated_bump(prospect_id: int, client_sdr_id: int, generated_message_auto_bump_id: int) -> bool:
    ...
    last_message: LinkedinConversationEntry = latest_convos[0]
    ...
    if last_message is None or message.latest_li_message_id != last_message.id:
        ...
        db.session.commit()
        send_slack_message(
            message=f"••• 🚨 Will not send because cached bump is out of date. Autogenerated bump message #{message.id} for prospect {prospect.full_name} (#{prospect.id}) discarded because it's not responding to the last message in the thread 🤖\nLast message: {last_message.message}\nMessage: {message.message}",
            ...
            blocks=[
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": "🚨 Will not send because cached bump is out of date",
                        "emoji": True
                    }
                },
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"Autogenerated bump message #{message.id} for prospect {prospect.full_name} (#{prospect.id}) discarded because it's not responding to the last message in the thread 🤖"
                    }
                },
                {
                    "type": "context",
                    "elements": [
                        {
                            "type": "mrkdwn",
                            "text": "*Most recent message:* " + last_message.message,
                        },
                    ],
                },
                {
                    ...
                    "elements": [
                        {
                            "type": "mrkdwn",
                            "text": "*Autobump Message:* " + message.message,
                        },
                    ],
                },
    ...
    if len(message.message) < 5:
        ...
        db.session.commit()
        send_slack_message(
            message=f"••• 🚨 Will not send because generated message is too short. Autogenerated bump message #{message.id} for prospect {prospect.full_name} (#{prospect.id}) discarded because the last message in the thread is too short 🤖\nLast message: {last_message.message}\nMessage: {message.message}",
            webhook_urls=[URL_MAP["operations-autobump"]],
            blocks=[
                {
                    "type": "header",
                    "text": {
                        "type": "plain_text",
                        "text": "🚨 Will not send because generated message is too short",
                        "emoji": True
                    }
                },
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"Autogenerated bump message #{message.id} for prospect {prospect.full_name} (#{prospect.id}) discarded because the message is too short 🤖"
                    }
                },
                {
                    "type": "context",
        ...
        blocks=[
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": "✅ TESTING TESTING Sending autogenerated bump message",
                    "emoji": True
                }
            },
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": f"Autogenerated bump message #{message.id} for prospect {prospect.full_name} (#{prospect.id}) 🤖"
                }
            },
            {
                "type": "context",
