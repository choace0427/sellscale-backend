...
def rule_default_framework(autobump_id: int, violations: list) -> tuple[bool, str]:
    ...
    if not bump_framework.default:
        violations.append("Bump framework is not default")
...
def rule_no_stale_message(autobump_id: int, violations: list) -> tuple[bool, str]:
    """Rule: Messages cannot be stale (generated more than 2 days ago)"""
    # Get the autobump
    autobump: GeneratedMessageAutoBump = GeneratedMessageAutoBump.query.get(
        autobump_id
    )
    if autobump.created_at < datetime.now() - timedelta(days=2):
        violations.append("Message is stale (generated more than 2 days ago)")
        return False, "Message is stale (generated more than 2 days ago)"
...
def rule_no_profanity(message: str, violations: list) -> tuple[bool, str]:
    ...
    if detected_profanities:
        violations.append("Message contains profanity: " + ", ".join(detected_profanities))
