...
from src.bump_framework_email.services import (
    ...
    deactivate_bump_framework_email,
    get_bump_framework_email_count_for_sdr,
    get_bump_frameworks_email_for_sdr,
    modify_bump_framework_email
)
...
BUMP_FRAMEWORK_EMAIL_BLUEPRINT = Blueprint("bump_framework_email", __name__)


@BUMP_FRAMEWORK_EMAIL_BLUEPRINT.route("/bump", methods=["GET"])
@require_user
def get_email_bump_frameworks(client_sdr_id: int):
    ...
    bump_frameworks: list[dict] = get_bump_frameworks_email_for_sdr(
    ...
    counts = get_bump_framework_email_count_for_sdr(
        client_sdr_id=client_sdr_id,
        client_archetype_ids=client_archetype_ids,
    )
...
@BUMP_FRAMEWORK_EMAIL_BLUEPRINT.route("/bump", methods=["POST"])
@require_user
def post_create_bump_framework(client_sdr_id: int):
    ...
    if bump_framework_id:
        return (
            jsonify(
                {
                    "message": "Successfully created email bump framework",
                    "bump_framework_id": bump_framework_id,
                }
            ),
            200,
        )
    return jsonify({"error": "Could not create email bump framework."}), 400


@BUMP_FRAMEWORK_EMAIL_BLUEPRINT.route("/bump", methods=["PATCH"])
@require_user
...
@BUMP_FRAMEWORK_EMAIL_BLUEPRINT.route("/bump/deactivate", methods=["POST"])
@require_user
def post_deactivate_bump_framework(client_sdr_id: int):
    ...
    deactivate_bump_framework_email(client_sdr_id, bump_framework_id)
    return jsonify({"message": "Bump framework deactivated."}), 200


@BUMP_FRAMEWORK_EMAIL_BLUEPRINT.route("/bump/activate", methods=["POST"])
@require_user
def post_activate_bump_framework(client_sdr_id: int):
    ...
    activate_bump_framework_email(client_sdr_id, bump_framework_id)
