...
def get_bump_frameworks_email_for_sdr(
    client_sdr_id: int,
    ...
    bfs: list[BumpFrameworkEmail] = BumpFrameworkEmail.query.filter(
        BumpFrameworkEmail.client_sdr_id == client_sdr_id,
        BumpFrameworkEmail.client_archetype_id.in_(client_archetype_ids),
        BumpFrameworkEmail.overall_status.in_(overall_statuses),
    )

    # If substatuses is specified, filter by substatuses
    if len(substatuses) > 0:
        bfs = bfs.filter(BumpFrameworkEmail.substatus.in_(substatuses))

    # If activeOnly is specified, filter by active
    if activeOnly:
        bfs = bfs.filter(BumpFrameworkEmail.active == True)

    bfs: list[BumpFrameworkEmail] = bfs.all()

    return [bf.to_dict() for bf in bfs]


def get_bump_framework_email_count_for_sdr(
    ...
    """
    bump_frameworks = get_bump_frameworks_email_for_sdr(
...
def create_bump_framework_email(
    client_sdr_id: int,
    ...
    return bump_framework_email_id


def modify_bump_framework_email(
    client_sdr_id: int,
    client_archetype_id: int,
    ...
    """
    bump_framework: BumpFrameworkEmail = BumpFrameworkEmail.query.filter(
        BumpFrameworkEmail.client_sdr_id == client_sdr_id,
        BumpFrameworkEmail.id == bump_framework_email_id,
    ).first()
    ...
    if default:
        ...
        if overall_status == ProspectOverallStatus.BUMPED and bumped_count is not None:
            default_bump_frameworks = default_bump_frameworks.filter(
                BumpFrameworkEmail.bumped_count == bumped_count
            )
...
def deactivate_bump_framework_email(client_sdr_id: int, bump_framework_id: int) -> None:
    ...
    """
    bump_framework: BumpFrameworkEmail = BumpFrameworkEmail.query.filter(
        BumpFrameworkEmail.id == bump_framework_id,
        BumpFrameworkEmail.client_sdr_id == client_sdr_id,
    ).first()
...
def activate_bump_framework_email(client_sdr_id: int, bump_framework_id: int) -> None:
