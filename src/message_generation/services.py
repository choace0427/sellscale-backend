...
from src.li_conversation.models import LinkedInConvoMessage, LinkedinInitialMessageTemplate
...
from src.ml.services import determine_best_bump_framework_from_convo, get_text_generation
...
def get_messages_queued_for_outreach(
    ...
    joined_prospect_message = (
        joined_prospect_message.order_by(
            OutboundCampaign.priority_rating.desc())
...
def process_generated_msg_queue(
    ...
    if li_message_urn_id:
        ...
        if msg_queue.bump_framework_id:
            bf: BumpFramework = BumpFramework.query.get(
                msg_queue.bump_framework_id)
            if bf:
...
def generate_followup_response(
    ...
    try:
        ...
        use_account_research = (
            best_framework.get(
                "use_account_research") if best_framework else True
        )
...
def generate_li_convo_init_msg(prospect_id: int, use_templates: bool = False):
    ...
    rp_values = [
        x.value for x in raw_research_points] if raw_research_points else []
...
Keep the whole message 1-2 sentences and 1 paragraph long. Keep it short!""",
    ...
    try:
        ...
        prospect = get_indiduals_prospect_id_from_linkedin_url(
            PROSPECT_LINKEDIN)
        send_slack_message(
        ...
        prospect_rp = ResearchPayload.query.filter_by(
            prospect_id=prospect).first()
        ...
        sdr_company_description = deep_get(
            user_rp, "company.details.description") or ""
        sdr_company_tagline = deep_get(
            user_rp, "company.details.tagline") or ""
        prospect_name = (
