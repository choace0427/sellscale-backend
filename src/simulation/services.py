...
from src.simulation.models import Simulation, SimulationType, SimulationRecord, SimulationRecordType


def create_simulation(client_sdr_id: int, archetype_id: int, prospect_id: int, type: SimulationType):
...
def send_li_convo_message(simulation_id: int, message: LinkedInConvoMessage, meta_data: Optional[dict] = None):
...
def get_sim_li_convo_history(simulation_id: int, fetchAll: bool = False) -> List[LinkedInConvoMessage]:
    '''
    ...
    if not fetchAll: query = query.limit(5)
...
def generate_sim_li_convo_init_msg(simulation_id: int):
  ...
  from src.message_generation.services_stack_ranked_configurations import get_top_stack_ranked_config_ordering, random_cta_for_prospect
  from src.message_generation.models import StackRankedMessageGenerationConfiguration
  from src.message_generation.services import generate_batch_of_research_points_from_config

  simulation: Simulation = Simulation.query.get(simulation_id)
  if not simulation: return False
  if simulation.type != SimulationType.LI_CONVERSATION: return False
  ...
  if len(research_points) == 0: return False
...
def generate_sim_li_convo_response(simulation_id: int) -> Tuple[bool, str]:
  ...
  simulation: Simulation = Simulation.query.get(simulation_id)
  if not simulation: return False, 'No simulation found.'
  if simulation.type != SimulationType.LI_CONVERSATION: return False, 'Wrong simulation type.'
  if simulation.meta_data is None: return False, 'Missing meta data.'

  convo_history = get_sim_li_convo_history(simulation_id)
  if len(convo_history) == 0: return False, 'No conversation history.'
  ...
  if overall_status is None or li_status is None or bump_count is None: return False, 'Missing meta data.'
  ...
  try:
    ...
    if data is None: return False, 'No bump framework found.'
    ...
    success = send_li_convo_message(
      ...
      meta_data={
        ...
        "bump_framework_description": data.get('bump_framework_description', None),
...
def update_sim_li_convo(simulation_id: int):
  ...
  first_and_only_message_was_you = len(convo_history) == 1 and convo_history[0].connection_degree == "You"
  ...
  last_msg_was_you = (
    len(convo_history) > 1
    and convo_history[0].connection_degree == "You"
  )
  last_2_msg_was_you = (
    len(convo_history) > 2
  ...
  has_prospect_replied = next(filter(lambda x: x.connection_degree != "You", convo_history), None) is not None
