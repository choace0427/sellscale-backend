...
def get_prospects_for_income_pipeline(client_sdr_id: int):
    ...
    data = db.session.execute(
        ...
        select
          ...
          array_agg(concat(prospect.id, ' -~- ', prospect.company, ' -~- ', prospect.company_url, ' -~- ', prospect.full_name, ' -~- ', prospect.title, ' -~- ', prospect.img_url, ' -~- ', 'company_img_url', ' -~- ', prospect.contract_size, ' -~- ', prospect.linkedin_url, ' -~- ', prospect.updated_at, ' -~- ', prospect_email.outreach_status, ' -~- ', prospect.status)) filter (where prospect.overall_status = 'ACTIVE_CONVO' and prospect.status != 'ACTIVE_CONVO_SCHEDULING' and (prospect_email.outreach_status != 'SCHEDULING' or prospect_email.outreach_status is null)) "ACTIVE_CONVO",
