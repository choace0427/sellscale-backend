...
def test_get_valid_next_prospect_statuses():
    ...
    assert (
        valid_next_statuses["valid_next_statuses"][ProspectStatus.SENT_OUTREACH.value]
        is not None
    )
    assert (
    ...
    assert (
        valid_next_statuses["valid_next_statuses"][ProspectStatus.ACCEPTED.value]
        is not None
    )
    assert (
        valid_next_statuses["valid_next_statuses"][ProspectStatus.RESPONDED.value]
        is not None
    )
    assert (
        valid_next_statuses["valid_next_statuses"][ProspectStatus.ACTIVE_CONVO.value]
        is not None
    )
    assert (
        valid_next_statuses["valid_next_statuses"][ProspectStatus.NOT_QUALIFIED.value]
