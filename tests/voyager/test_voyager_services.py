...
@mock.patch("src.voyager.services.chat_ai_classify_active_convo", return_value=ProspectStatus.ACTIVE_CONVO_CIRCLE_BACK)
def test_get_prospect_status_from_convo(mock_chat_ai_classify_active_convo, mock_chat_ai_verify_scheduling_convo):
    client = basic_client()
    sdr = basic_client_sdr(client)
    sdr.name = "David"

    messages = ["David (7/4): Hey Aakash, would you like to buy an AI solution from me",
    ...
    with mock.patch("src.voyager.services.chat_ai_verify_scheduling_convo", return_value=False) as mock_chat_ai_verify_scheduling_convo:
        messages = ["David (7/4): Hey Aakash, would you like to buy an AI solution from me",
